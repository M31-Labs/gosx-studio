...
func TestReadinessView(t *testing.T) {
	readiness := NewShellReadiness(
		NewShellReadinessItem("Studio shell", "Studio shell", ShellReadinessReady, "Mounted", "Canvas and rails are ready."),
		NewShellReadinessItem("media", "Media", ShellReadinessWatch, "Alt text", "One asset needs alt text.").WithHref("/admin/media"),
		NewShellReadinessItem("calendar", "Calendar", ShellReadinessNext, "Needed for Pajaritos", "Register calendar recipes."),
		ShellReadinessItem{Key: "skip"},
	)
	if readiness.Summary() != "1/3 ready" {
		t.Fatalf("unexpected summary: %s", readiness.Summary())
	}
	view := ShellReadinessView(readiness)
...
func TestView(t *testing.T) {
	shell := New(Options{
		...
		Readiness:     NewShellReadiness(NewShellReadinessItem("shell", "Shell", ShellReadinessReady, "Mounted", "Ready.")),
	})
	view := View(shell)

...
func TestSiteMapAuthoringViewProjectsInteractiveEditorPayload(t *testing.T) {
	...
	if metadataPage["authoringOperation"] != "update-page" || metadataForm[authoring.AuthoringFieldPageKey] != "home" {
	...
	if len(metadataInputs) == 0 || metadataInputs[0]["name"] != authoring.AuthoringFieldOperation || metadataInputs[0]["value"] != "update-page" {
	...
	if !authoringFormInputsContain(moveDownInputs, authoring.AuthoringFieldPosition, "1") {
	...
	if view["hasReorderComponent"] != true || reorderPage["authoringOperation"] != "reorder-component" || reorderForm[authoring.AuthoringFieldPosition] != "1" {
		t.Fatalf("unexpected reorder component payload: %#v", reorderPage)
	}
	reorderInputs := reorderPage["formInputs"].([]map[string]string)
	if !authoringFormInputsContain(reorderInputs, authoring.AuthoringFieldPosition, "1") {
	...
	if view["hasVisibilityComponent"] != true || visibilityPage["authoringOperation"] != "toggle-visibility" || visibilityForm[authoring.AuthoringFieldVisible] != "false" {
	...
	if !authoringFormInputsContain(visibilityInputs, authoring.AuthoringFieldVisible, "false") {
	...
	if view["hasDeleteComponent"] != true || deletePage["authoringOperation"] != "delete-component" || deleteForm[authoring.AuthoringFieldComponentKey] != "hero" {
		t.Fatalf("unexpected delete component payload: %#v", deletePage)
	}
	deleteInputs := deletePage["formInputs"].([]map[string]string)
	if !authoringFormInputsContain(deleteInputs, authoring.AuthoringFieldComponentKey, "hero") {
	...
	if view["hasDuplicateComponent"] != true || duplicatePage["authoringOperation"] != "duplicate-component" || duplicateForm[authoring.AuthoringFieldComponentTemplateKey] != "hero" || duplicateForm[authoring.AuthoringFieldPosition] != "1" {
	...
	if !authoringFormInputsContain(duplicateInputs, authoring.AuthoringFieldComponentTemplateKey, "hero") {
	...
	if form[authoring.AuthoringFieldOperation] != "save-control" || form[authoring.AuthoringFieldPageKey] != "home" || form[authoring.AuthoringFieldComponentKey] != "hero" {
		t.Fatalf("unexpected control form payload: %#v", form)
	}
	controlInputs := headline["formInputs"].([]map[string]string)
	if !authoringFormInputsContain(controlInputs, authoring.AuthoringFieldBinding, "pages.home.hero.headline") {
	...
	if view["hasEditableControl"] != true || editableControl["key"] != "headline" || editableControl["componentKey"] != "hero" || editableControl["inputName"] != authoring.AuthoringFieldValue {
		t.Fatalf("unexpected editable control projection: %#v", editableControl)
	}
	if editableControl["actionLabel"] != "Save field" || editableControlForm[authoring.AuthoringFieldBinding] != "pages.home.hero.headline" || editableControlForm[authoring.AuthoringFieldValue] != "Forest days" {
	...
	if !authoringFormInputsContain(editableControlInputs, authoring.AuthoringFieldControlKey, "headline") {
	...
	if addForm[authoring.AuthoringFieldOperation] != "apply-intent" || addForm[authoring.AuthoringFieldComponentTemplateKey] != "gallery" {
		t.Fatalf("unexpected add intent form: %#v", addForm)
	}
	addInputs := add["formInputs"].([]map[string]string)
	if len(addInputs) == 0 || addInputs[0]["name"] != authoring.AuthoringFieldOperation || addInputs[0]["value"] != "apply-intent" {
		t.Fatalf("expected ordered add intent form inputs: %#v", addInputs)
	}
	if !authoringFormInputsContain(addInputs, authoring.AuthoringFieldComponentTemplateKey, "gallery") {

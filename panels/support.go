...
// This file carries the block-layout open-tag renderer and the site-map
// hidden-input/map-shape helpers that the panel files (advanced_panel.go,
// brand_panel.go, home_layers_panel.go, inspector.go, inspector_fields.go, …)
// need. panels may import only core and authoring (§1 Import DAG); the
// canonical copies live in canvas (Slice 4) and sitemap (Slice 5), both peers,
// so importing either would be a forbidden peer import.
//
...
// The remaining frozen copies (renderBlockLayoutEngineOpenTag vs.
// canvas.RenderBlockLayoutEngineOpenTag, and the siteMap* hidden-input helpers
// vs. sitemap's exported equivalents) stay until a later slice gives them a
// shared home; each is covered by this package's own render tests plus the
// canonical package's tests.
